"""Standalone leave-one-city-out training script (run out-of-notebook for live, tailable logging).

Rebuilds the exact same data pipeline as part2_segmentation.ipynb sections 1-3, then trains the
leave-one-city-out model. Saves the best checkpoint + per-epoch history so the notebook can load
results back in without retraining. See PART2_PLAN.md for the design rationale.
"""
...
HOLDOUT_CITY = "kitsap"
TRAIN_IDS, VAL_IDS = SUBSET_IDS[:6], SUBSET_IDS[6:7]
...
def primary_role(tid):
    if tid in TRAIN_IDS: return "train"
    if tid in VAL_IDS: return "val"
    return "test"

patch_index["split"] = patch_index["tile_id"].map(primary_role)
...
log(f"LOCO: train={loco_train_mask.sum()} val={loco_val_mask.sum()} test(all of {HOLDOUT_CITY})={loco_test_mask.sum()}")
...
loco_train_ds = PatchDataset(patch_index[loco_train_mask], train_tf)
loco_val_ds = PatchDataset(patch_index[loco_val_mask], eval_tf)
log(f"loco_train={len(loco_train_ds)} loco_val={len(loco_val_ds)}")
...
if __name__ == "__main__":
    model, history = run_training(loco_train_ds, loco_val_ds, epochs=30, batch_size=16, tag="loco")
    torch.save(model.state_dict(), ARTIFACTS / "loco_model.pt")
    history.to_csv(ARTIFACTS / "loco_history.csv", index=False)
